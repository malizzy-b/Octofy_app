...
class OctofyAppSupabaseUser extends BaseAuthUser {
  OctofyAppSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> octofyAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OctofyAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
